	...
	<% 
	 ...
	 <table>
         <tr>  <!-- << 에 1페이지를 연결해서 <<을 누르면 1페이지로 돌아감.-->
            <td style=text-align:center; width=20px;><a href="./e_01.jsp?pageNum=1">&lt;&lt;</a></td>
            <!-- < 에 연될될 페이지  -->
            <td style=text-align:center; width=20px;><a href="./e_01.jsp?pageNum=<%=nPage%>">&lt;</a></td>
    <%         
        for(int i = startPage; i <= endPage; i++){  /* i페이지에 연결될 url*/
    %>      <td style=text-align:center; width=20px;><a href="./e_01.jsp?pageNum=<%=i%>"><%= i %></a></td>
    <%
        }           
        if(endPage < pageCount){  /* > 에 연결될 페이지 << >> 사이에 마지막 페이지가 총 페이지 보다 작을 때  */
    %>
            <td style=text-align:center; width=20px;><a href="./e_01.jsp?pageNum=<%=startPage+pageBlock%>">&gt;</a></td>
    <%		
        } else {   /* > 에 연결될 페이지 << >> 사이에 마지막 페이지가 총 페이지 보다 작지 않을 때  */
    %>
    		<td style=text-align:center; width=20px;><a href="./e_01.jsp?pageNum=<%=pageCount%>">&gt;</a></td>    		
    <%		
        } 
    %>
