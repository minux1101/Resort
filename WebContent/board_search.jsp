...
<body>
	...
	if(pageNum != "1") {  //페이지 넘버가 없으면 시작페이지는 1, 시작 행은 0
	startNum = currentPage * cnt - cnt;
	}
	List<Board> boardSearch = bs.search(condition, sh, startNum, cnt); //Board 타입의 list 생성	
	List<Board> Search = bs.searchAll(condition, sh); //Board 타입의 list 생성	
	...
	if(boardSearch.size() > 0) {  /*list에 값이 있을 때*/
		for (Board board : boardSearch) {  /*list가 끝날때까지 for문 수행*/
			String hyphen = "";
	...
	<tr>
	 ...
	 <table>
         <tr>  <!-- << 에 1페이지를 연결해서 <<을 누르면 1페이지로 돌아감.-->
            <td style=text-align:center; width=20px;><a href="./board_search.jsp?sch=<%=condition%>&search=<%=sh%>&pageNum=1  ">&lt;&lt;</a></td>
            <!-- < 에 연될될 페이지  -->
            <td style=text-align:center; width=20px;><a href="./board_search.jsp?sch=<%=condition%>&search=<%=sh%>&pageNum=<%=nPage%>">&lt;</a></td>
    <%         
        for(int i = startPage; i <= endPage; i++){  /* i페이지에 연결될 url*/
    %>      <td style=text-align:center; width=20px;><a href="./board_search.jsp?sch=<%=condition%>&search=<%=sh%>&pageNum=<%=i%>"><%= i %></a></td>
    <%
        }           
        if(endPage < pageCount){  /* > 에 연결될 페이지 */
    %>
            <td style=text-align:center; width=20px;><a href="./board_search.jsp?sch=<%=condition%>&search=<%=sh%>&pageNum=<%=startPage+pageBlock%>">&gt;</a></td>
    <%		
        } else {
    %>
    		<td style=text-align:center; width=20px;><a href="./board_search.jsp?sch=<%=condition%>&search=<%=sh%>&pageNum=<%=pageCount%>">&gt;</a></td>    		
    <%		
        } 
    %>
