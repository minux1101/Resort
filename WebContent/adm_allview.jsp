...
<script>
    ...
    var calendar = new FullCalendar.Calendar(calendarEl, {
      ...
      events: [      
          ...
          <% } else {  %>
        	  {
                  title: '스위트 <%=rs.selectOne(sweets.get(i).getDate(), 1).getName()%>',
                  start: '<%=sweets.get(i).getDate()%>',
                    end: '<%=sweets.get(i).getDate()%>'
                  },
         <% }
          ...
          <%    } else { %>
          {
              title: '패밀리 <%=rs.selectOne(familes.get(i).getDate(), 2).getName()%>',
              start: '<%=familes.get(i).getDate()%>',
                end: '<%=familes.get(i).getDate()%>'
              },        	  
          <%}
         ...
         <%    } else { %>
         {
             title: '디럭스 <%=rs.selectOne(deluxes.get(i).getDate(), 3).getName()%>',
             start: '<%=deluxes.get(i).getDate()%>',
               end: '<%=deluxes.get(i).getDate()%>'
           },
       <%  } 
